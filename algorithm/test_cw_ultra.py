...
def test_cw_ultra():
    ...
    print("="*80)
    print("ULTRA TEST WITH CLARKE-WRIGHT")
    print("="*80)
    print(f"Instances: {len(target_instances)}")
    print(f"Time limit: 180s per instance")
    print(f"Initial solution: CLARKE-WRIGHT (not Greedy)")
    print("="*80)
    ...
    for idx, inst_name in enumerate(target_instances, 1):
        ...
        try:
            ...
            # Save progress
            with open('cw_ultra_results.json', 'w') as f:
    ...
    if results:
        ...
        if avg_gap <= 25:
            ...
            print(f"\n  Target not achieved. Gap {avg_gap:.1f}% > 25%")
