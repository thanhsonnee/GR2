"""
ULTRA AGGRESSIVE TEST - With Route Improvement
Target: gap <=5% for multiple instances, avg gap <=10%

New improvements:
- Route improvement (2-opt, relocate, exchange)  
- Aggressive AGES (better vehicle reduction)
- Time limit: 180s per instance (3 minutes)
- LNS: 3000 iterations
- Destroy: 10-60
"""
...
def test_ultra_aggressive():
    ...
    print("=" * 80)
    print("ULTRA AGGRESSIVE TEST - With Route Improvement")
    print("=" * 80)
    print(f"Instances: {len(target_instances)}")
    print(f"Time limit: 180s per instance")
    print(f"Config: LNS 3000 iter, Destroy 10-60, ILS 25 iter")
    print(f"NEW: Route improvement (2-opt, relocate, exchange)")
    print("=" * 80)
    ...
    for idx, inst_name in enumerate(target_instances, 1):
        ...
        try:
            ...
            # Run ILS with ULTRA AGGRESSIVE parameters
            start_time = time.time()
            ils = IteratedLocalSearch(
                instance,
                max_iterations=25,  # Even more iterations
                max_time=180,  # 3 minutes per instance
                no_improvement_limit=999  # No early stopping
            )
            result = ils.solve()
    ...
    # Save results
    with open('ultra_aggressive_results.json', 'w') as f:
        json.dump(results, f, indent=2)
    
    # Print summary
    print("\n" + "=" * 80)
    print("ULTRA AGGRESSIVE TEST SUMMARY")
    print("=" * 80)
    ...
    if results:
        ...
        if gaps_dist:
            ...
            print(f"{'='*80}")
            print(f"Target 1: Gap <=5% for 2-5 instances")
            print(f"  Result: {excellent} instances {'✓' if excellent >= 2 else 'X'}")
            print(f"Target 2: Average gap <=10%")
            print(f"  Result: {avg_gap:.1f}% {'✓' if avg_gap <= 10 else 'X'}")
